"""
This file contains the evaluation  pipeline
for the two-stage LSTM NER classification
"""


from  utils_entities import *
...
class Net(nn.Module):
    def __init__(self, embedding_size, pos_tags, chunks, caps_dim, char_emb_size, bio_size,  device):
        ...
        self.fc_bio         = nn.Linear(bio_size, 20)
        ...
        self.lstm = nn.LSTM(250, 50, batch_first=True, dropout=0.2, num_layers=4, bidirectional=False)

        
        self.fc = nn.Linear(50, 5) #3 for O, PER, MISC, ORG, LOC
    
    def forward(self, s, tags, chunks, caps, p1,p2,p3, bio):
        ...
        p3 = self.p3(p3)
        b  = self.fc_bio(bio)
        s  =  torch.cat((s,t,c,caps, p1,p2,p3,b),2)
...
"""
def get_batch(data, pos_tags, chunks, caps, p1,p2,p3,bio, labels, pos_dim, chunks_dim):
...
#        print("max_idx", max_idx)
        ...
        batch_bio     = 0   * np.ones((num_of_sent, batch_max_len, 3)) # bio dim is 3 for one hot (i.e. B:[1,0,0], I:[0,1,0], O:[0,0,1])
...
#            print("bio len", len(bio[j]))
            batch_bio[j][:cur_len]=bio[j]
            
            
        return batch_data, batch_pos, batch_chunks, batch_caps, batch_p1, batch_p2, batch_p3, batch_bio, batch_labels, max_idx+7, charmax+2 #return.max_idx+1 is simply in order to initialize the embedding of the network later on.
...
"""
def split_to_batches(data, pos_tags, chunks, caps, p1,p2,p3,bio, labels, pos_dim,  chunks_dim, b_num):
    batch_x, batch_pos, batch_chunk, batch_capitals, batch_p1, batch_p2, batch_p3, batch_bio, batch_y, _, _ = get_batch(data, pos_tags, chunks, caps, p1,p2,p3,bio, labels,  pos_dim, chunks_dim) #get batched data in torch tensors.
...
#
    ...
    for i in range(batches): #split in batches
        ...
        bbio =batch_bio[i*b_num:(i*b_num + b_num)]
        ...
        batched_bio.append(bbio)

    return torch.LongTensor(batched_x), torch.LongTensor(batched_y),  torch.FloatTensor(batched_pos), torch.FloatTensor(batched_chunks), torch.FloatTensor(batched_caps), torch.LongTensor(batched_p1),torch.LongTensor(batched_p2),torch.LongTensor(batched_p3), torch.FloatTensor(batched_bio)
...
def evaluate(model, raw_data_filename, test_data_file):
    with torch.no_grad():
        ...
        x_test, x_pos, x_chunks, x_caps, x_p1, x_p2, x_p3, x_bio, y_test, _, pos_dim, chunks_dim = get_only_test_data_with_idxs(raw_data_filename, test_data_file)
        batch_x,batch_pos, batch_chunks, batch_caps, batch_p1, batch_p2, batch_p3, batch_bio, batch_y, max_dim,_ = get_batch(x_test, x_pos, x_chunks, x_caps, x_p1,x_p2, x_p3,x_bio, y_test, pos_dim, chunks_dim)
        ...
        t = model(batch_x, batch_pos, batch_chunks, batch_caps, batch_p1, batch_p2, batch_p3, batch_bio)
...
#        {'O': 0, 'ORG': 1, 'MISC': 2, 'PER': 3, 'LOC': 4}
        ...
        if tporg > 0:
            print("ORG label: Precision: ", (tporg/(tporg+fporg)), " Recall: ", (tporg/(tporg+fnorg)))
        k=0
        tporg=0; fporg=0; fnorg=0;
        for i, b in enumerate(batch_y):
            for idx, l in enumerate(b):
...
#        {'O': 0, 'ORG': 1, 'MISC': 2, 'PER': 3, 'LOC': 4}
        ...
        if tporg > 0:
            print("MISC label: Precision: ", (tporg/(tporg+fporg)), " Recall: ", (tporg/(tporg+fnorg)))
...
#        {'O': 0, 'ORG': 1, 'MISC': 2, 'PER': 3, 'LOC': 4}
        ...
        if tporg > 0:
            print("PER label: Precision: ", (tporg/(tporg+fporg)), " Recall: ", (tporg/(tporg+fnorg)))
...
#        {'O': 0, 'ORG': 1, 'MISC': 2, 'PER': 3, 'LOC': 4}
        ...
        if tporg > 0:
            print("LOC label: Precision: ", (tporg/(tporg+fporg)), " Recall: ", (tporg/(tporg+fnorg)))
...
def main():
    ...
    x_train, x_pos, x_chunks, x_caps, x_p1, x_p2, x_p3, x_bio, y_train, _, pos_dim, chunks_dim = get_train_test_data_with_idxs(raw_data_filename)

    batched_x, batched_y, batched_pos, batched_chunks, batched_capitalisations, batched_p1, batched_p2, batched_p3, batched_bio = split_to_batches(x_train, x_pos, x_chunks, x_caps, x_p1, x_p2, x_p3, x_bio, y_train, pos_dim, chunks_dim ,batches)


    batch_x,batch_pos, batch_chunks, batch_caps, batch_p1, batch_p2, batch_p3, batch_bio, batch_y, max_dim, char_emd_dim = get_batch(x_train, x_pos, x_chunks, x_caps,x_p1,x_p2,x_p3, x_bio, y_train, pos_dim, chunks_dim)
    ...
    print("Device:", device)
    ####---------------------------------TRAIN---------------------------###
    lstm_net = Net(max_dim, pos_dim,chunks_dim, 2, char_emd_dim, 3, device).to(device) #2 is  for the capitalisation one hot encoding dimension
    ...
    batched_bio=batched_bio.to(device)
    

    print(batched_x.shape)
    eval_file="eng.testa"
    lstm_net.load_state_dict(torch.load('lstm1entities.pt', map_location=torch.device('cpu')))
    losses = []
    ...
    for epoch in range(30):
        ...
        for idx, bx in enumerate(batched_x):
            if idx < 100:
                optimizer.zero_grad()
                out  = lstm_net(bx.to(device), batched_pos[idx].to(device), batched_chunks[idx].to(device), batched_capitalisations[idx].to(device), batched_p1[idx].to(device),batched_p2[idx], batched_p3[idx], batched_bio[idx])
        ...
        torch.save(lstm_net.state_dict(), 'lstm1entities.pt')
        joblib.dump(losses, "losses_entities_lstm.pkl")
