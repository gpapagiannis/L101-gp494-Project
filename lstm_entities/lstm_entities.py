...
#        print("max_idx", max_idx)
        ...
        for j in range(num_of_sent):
            ...
            batch_chunks[j][:cur_len] = chunks[j] # Do the  same for chunks. Note that tags are in one-hot-encoded versions
...
def loss_fn(outputs, labels, device):
    ...
    for idx, o in enumerate(outputs):
        for i, tok in enumerate(o):
            if random.random() < 0.90 and labels[idx, i] == 0: #UPDATE MAX LIK OF O LABEL 10% OF THE TIMES TO MITIGATE CLASS IMBALANCE
                out[idx, i]= 0
            else:
...
def main():
    ...
    for i in range(1):
        ...
        eval_file="eng.testb"
#        lstm_net.load_state_dict(torch.load("lstm_entities_seed_{}.pt".format(i), map_location=torch.device('cpu')))
        losses = []
        ...
        for epoch in range(1):
            ...
            for idx, bx in enumerate(batched_x):
                if idx < 27:
                    ...
                    optimizer.step()
                    losses.append(loss)
            org, misc, per, loc, other = evaluate(lstm_net, raw_data_filename, eval_file)
            forg.append(org[0])
            fmisc.append(misc[0])
            fper.append(per[0])
            floc.append(loc[0])
    ...
    #        torch.save(lstm_net.state_dict(), "lstm_entities_seed_{}.pt".format(seed))
    #        joblib.dump(losses, "losses_entities_lstm_{}.pkl".format(seed))
